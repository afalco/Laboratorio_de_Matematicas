...
\begin{itemize}
  \item distinguir entre la consola, un script y un proyecto de R;
  \item organizar los archivos de una práctica de forma reproducible;
  \item instalar y cargar paquetes sin confundir ambas operaciones;
  \item consultar la ayuda integrada y obtener información sobre la sesión de trabajo.
\end{itemize}
\end{objetivos}
...
\begin{itemize}
  \item La \textbf{consola} ejecuta instrucciones inmediatamente. Es útil para explorar, pero no conserva por sí sola una explicación completa del trabajo.
  \item Un \textbf{script} es un archivo de texto, normalmente con extensión \texttt{.R}, que contiene instrucciones en un orden explícito. Es el lugar habitual para guardar el código de una práctica.
  \item Un \textbf{proyecto} asocia una carpeta con el trabajo de R. Al abrirlo, RStudio puede recuperar la carpeta del proyecto y facilita que las rutas relativas sean estables.
\end{itemize}
...
La función \texttt{file.path()} construye rutas respetando el sistema operativo. Esta práctica es más robusta que escribir manualmente separadores como \texttt{/} o \texttt{\\}.

\begin{aviso}
Evita comandos como \texttt{setwd("C:/Users/...")} en los scripts que se entregan. Funcionan solo en el ordenador donde se escribieron y dificultan la reproducción del análisis.
\end{aviso}
...
Los comentarios comienzan con \texttt{\#}. Deben explicar una decisión o separar partes lógicas del programa, no repetir literalmente lo que ya dice el código.
...
\section{Paquetes y bibliotecas}
La instalación de un paquete se hace normalmente una sola vez por versión de R. Cargarlo con \texttt{library()} es una operación distinta y suele hacerse al comienzo de cada sesión o script.
...
Para comprobar si un paquete está disponible sin detener inmediatamente el script se puede utilizar \texttt{requireNamespace()}.
...
\begin{enumerate}
  \item crear o abrir un proyecto para la asignatura;
  \item leer el enunciado y escribir una breve descripción del objetivo;
  \item guardar el código en un script, ejecutándolo por bloques pequeños;
  \item comprobar los resultados con ejemplos sencillos y casos límite;
  \item separar datos originales, figuras, resultados y código;
  \item redactar la interpretación junto al resultado que la justifica;
  \item ejecutar el script desde una sesión limpia antes de entregar.
\end{enumerate}
...
Al finalizar este capítulo, el estudiante debe ser capaz de:
\begin{itemize}
  \item identificar el tipo y la estructura básica de un objeto de R;
  \item utilizar operadores aritméticos, relacionales y lógicos;
  \item detectar conversiones automáticas y valores especiales;
  \item inspeccionar objetos antes de operar con ellos y documentar supuestos.
\end{itemize}
\end{objetivos}

\section{Objetos y asignación}
Un objeto es un nombre asociado a un valor. En R se recomienda usar \texttt{<-} para asignar. La asignación crea o actualiza un nombre; la expresión situada a la derecha se evalúa antes de guardar el resultado.
...
Los nombres deben ser descriptivos. \texttt{area\_circulo} es preferible a \texttt{a} cuando el contexto no es inmediato. En un mismo proyecto conviene elegir una convención y mantenerla: por ejemplo, minúsculas y guiones bajos.
...
\texttt{ls()} muestra los nombres disponibles y \texttt{rm()} elimina objetos. No se recomienda usar \texttt{rm(list = ls())} dentro de un script docente: borra el contexto completo y puede ocultar dependencias no declaradas.
...
Los tipos atómicos más habituales son \texttt{integer}, \texttt{double} o numérico, \texttt{character}, \texttt{logical} y \texttt{complex}. La clase que devuelve \texttt{class()} es una descripción orientada al uso; \texttt{typeof()} informa del tipo interno.
...
Para explorar un objeto antes de utilizarlo son especialmente útiles \texttt{str()}, \texttt{length()} y \texttt{names()}.
...
\section{Operadores y precedencia}
Las expresiones combinan valores y operadores. Los operadores aritméticos principales son \texttt{+}, \texttt{-}, \texttt{*}, \texttt{/}, \texttt{\^} y \texttt{\%\%}. La división entera se obtiene con \texttt{\%/\%}.
...
Las comparaciones producen valores lógicos: \texttt{==}, \texttt{!=}, \texttt{<}, \texttt{<=}, \texttt{>} y \texttt{>=}. Los operadores \texttt{\&} y \texttt{|} combinan condiciones elemento a elemento; \texttt{\&\&} y \texttt{||} se reservan para decisiones escalares, especialmente en condicionales.
...
\section{Conversión y reciclaje}
...
La función \texttt{as.numeric()} aplicada a texto no numérico produce \texttt{NA} y un aviso. Conviene comprobar siempre el resultado de una conversión.
...
\section{Valores especiales y datos ausentes}
Los valores \texttt{NA}, \texttt{NaN}, \texttt{Inf} y \texttt{-Inf} no significan lo mismo:
\begin{itemize}
  \item \texttt{NA} representa un dato ausente o desconocido.
  \item \texttt{NaN} representa un resultado numéricamente indefinido, como \texttt{0/0}.
  \item \texttt{Inf} y \texttt{-Inf} representan infinitos con signo.
\end{itemize}
...
La opción \texttt{na.rm = TRUE} elimina los valores ausentes para esa operación, pero no explica por qué faltan. En un análisis serio hay que distinguir entre eliminar datos, imputarlos o conservar explícitamente la ausencia.
...
En cálculo computacional conviene distinguir igualdad matemática exacta e igualdad numérica aproximada. Para comparar resultados con tolerancia se puede usar \texttt{all.equal()} o una desigualdad explícita.
...
\section{Atributos e inspección}
...
\begin{enumerate}
  \item ¿Qué tipo y longitud tiene cada objeto?
  \item ¿Las unidades y magnitudes son compatibles?
  \item ¿Hay valores ausentes, infinitos o no numéricos?
  \item ¿Las comparaciones requieren una tolerancia?
  \item ¿La operación está reciclando un vector de forma intencionada?
\end{enumerate}
